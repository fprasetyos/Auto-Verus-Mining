...
# echo -e "Pool address : "$(cat $HOME_DIR/autominer.log | grep "Starting on" | awk '{print $5}')"" >> $MSG_FILE
# echo -e "Hashrate : "$(tail -1 $HOME_DIR/autominer.log | grep "accept" | awk '{print $7}')" kH/s" >> $MSG_FILE
# echo -e "Last share : "$(tail -1 $HOME_DIR/autominer.log | grep "accept" | awk '{print $4}') >> $MSG_FILE

# Send Notif Telegram
TOKEN=1872008021:AAEJyVLt2pk2Tc5viMDyNqh8DNM0mSB1k6w
CHAT_ID=(1613484808) #ID_TELEGRAM diisi dengan id yang akan menerima notifnya.
